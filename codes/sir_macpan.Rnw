...
params <- update(params, c(beta0=sir.params[["beta"]],
                           gamma_a=sir.params[["gamma"]],
                           gamma_m=sir.params[["gamma"]],
                           gamma_s=sir.params[["gamma"]],
                           gamma_p=sir.params[["gamma"]]))
...
sim_combined <- sim0 %>% mutate(sir.S=sir.out[["S"]],sir.I=sir.out[["I"]],sir.S=sir.out[["R"]],
                                sir.incidence=sir.params[["beta"]]*sir.out[["S"]]*sir.out[["I"]])

gg01 <- (ggplot(sim_combined,aes(x=date))
+ geom_point(aes(y=I))
+ geom_point(aes(y=sir.I), color='red')
)
print(gg01)

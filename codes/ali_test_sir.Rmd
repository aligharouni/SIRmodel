...
df2 <- df2 %>% dplyr::mutate(R0_sub = ifelse(eta_w<eta_t, NA, R0))  

df2_fake <- df2 %>% dplyr::select(eta_t,eta_w)
df2_fake <- unique(df2_fake) %>% dplyr::mutate(flag=ifelse(eta_t<=eta_w,1,0))
```
...
p1_temp <- (p1
    ...
    + geom_rect(data=df2,
                          fill=ifelse(df2$eta_t>df2$eta_w,"white","NA" ),
                          color= NA,
                          ymin=-1,
                          ymax=10,
                          xmin=-1,
...
#                           fill=ifelse(df2_fake$eta_t>df2_fake$eta_w,"white","black"),
#                           color= NA,
#                           ymin=-1,
#                           ymax=10,
#                           xmin=-1,

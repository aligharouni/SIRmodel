...
```{r contour_R0_2, message=FALSE,warning=FALSE }
 # specify the ranges
beta  <- 0.35  ## set beta high enough to allow R0>1 in worst case
 eta_w <- seq(0,1, length.out=n_out)
 rho     <- seq(0,0.01, length.out=n_in)
 omega <- seq(0.1,1.5 , length.out=n_in) #note omega must be > rho, was 0.011
 
eta_t <- seq(0.001,0.01, length.out=n_out)  

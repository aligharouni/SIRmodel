...
# Case #2: 
all.equal(R0(params=update(params,eta_c=1,eta_w=1)), params[["beta"]]/params[["gamma"]])
```
...
# df2_fake <- df2 %>% dplyr::select(eta_c,eta_w)
# df2_fake <- unique(df2_fake) %>% dplyr::mutate(flag=ifelse(eta_c<=eta_w,1,0))
...
p1 <- ggplot(df2,aes(x=omega,y=rho,z=R0_sub))+ theme_bw() +
  ...
  ylab(TeX('$\\rho$, testing intensity (1/day per capita)')) +
    # theme(panel.spacing=grid::unit(0,"lines")) # initial theme, comment it for TTI testing plot
    theme(panel.spacing=grid::unit(0,"lines"),legend.position = "none") #AG, uncomment for Random Case
# p1_temp <- (p1
#     + geom_contour_filled(breaks=pretty(unique(df2$R0_sub),5),
...
#     + geom_rect(data=df2,
#                           fill=ifelse(df2$eta_c>df2$eta_w,"white","NA" ),
#                           color= NA,
#                           ymin=-1,
#                           ymax=10,
#                           xmin=-1,
...
p1_temp
# ## hack (By Dr. Ben Bolker) white rectangle to cover grid
#   ggplot() + 
#   scale_x_continuous(name="x") + 
#   scale_y_continuous(name="y") +
#   facet_grid(eta_w~eta_c, labeller=label_special) +
#     geom_rect(data=df2_fake,
#                           fill=ifelse(df2_fake$eta_c>df2_fake$eta_w,"white","black"),
#                           color= NA,
#                           ymin=-1,
#                           ymax=10,
#                           xmin=-1,
...
ggsave(p1_temp,filename = "R0contour_random.png" , path="../pix/")
...
R0_omega <- function(params){
  unpack(as.list(params))
         # a <- (eta_w-1)*W_I-(eta_c-1)*p_I*W_I
         a <- ((s-p_I)*eta_c + (p_I-1))*W_I  #involving s
...
# eta_w <- seq(0,1, length.out=n_out)
eta_c <- seq(0.001,1, length.out=n_out)

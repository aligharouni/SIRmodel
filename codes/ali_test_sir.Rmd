...
1- What combination of testing intensity, $t$, and isolation, $\eta_w$ and $\eta_t$, will result in $R_0 <1$?
...
sir.model <- function(time,state,params){
    ...
    ## scaling the weights
    sc <- tau*t*N0/(tau*(W_s*S_u+W_i*I_u+W_r*R_u)+t*N0)
    F_s <- sc*W_s
    F_i <- sc*W_i
    F_r <- sc*W_r
...
params <- c(N0=1000000, beta=0.8,gamma=1/3, omega=0.25, t=0.01, 
...
all.equal(sum(roots_dfe$root),params[["N0"]])

# DFE; analytic approach
Su_dfe <-function(params){
  unpack(as.list(params))
  a <- t
  b <- -t+(1-t/omega)*tau*W_s
  c <- -tau*W_s
  d <- b^2-4*a*c
  
  return(N0*2*a/(-b+sqrt(d)))
}
...
\label{exp:dfe} $S_n^*= \frac{t}{\omega} N_0, S_u^*= N_0-S_n^*$, and $I_j=R_j=0 \ \forall j$.
...
R0<-function(params){
    ...
    Sn_dfe<- N0-Su_dfe
    F_i <- tau*t*N0*W_i/(tau*(W_s*Su_dfe)+t*N0) #at DFE
...
```{r contour_R0_1, message=FALSE,warning=FALSE }
 # specify the ranges
 beta  <- 0.5  ## set beta high enough to allow R0>1 in worst case
 eta_w <- seq(0,1, length.out=5)
 eta_t <- seq(0.001,0.5, length.out=5) 
 t     <- seq(0,0.01, length.out=10)
 omega <- seq(0.1,1 , length.out=10) #note omega must be > t
# s <- seq(1,2.5,length.out=10) 
...
s <- seq(1,2.5,length.out=10) 
 ...
 df11 <- expand.grid(N0=params[["N0"]],beta=beta,gamma=params[["gamma"]],omega=omega,t=t,
                    W_s=params[["W_s"]],W_i=params[["W_i"]],W_r=params[["W_r"]],
                    P_s=params[["P_s"]],P_i=params[["P_i"]],P_r=params[["P_r"]],
...
p11 <- ggplot(df22,aes(x=omega,y=t,z=R0))+ theme_bw() +
    theme(panel.spacing=grid::unit(0,"lines"))
(p11
...
(1) Looking at the first column of panels, there seems to be a compensatory relation between $t$ and $\omega$. i.e., higher $t$ requires smaller $\omega$ to keep R0 lower than 1. Also, for a fixed $\omega$, increasing $\eta_w$ requires higher $t$ to achieve the critical R0.

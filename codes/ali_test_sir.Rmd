...
n_in <- 41 ## grid N within facets
 ...
 eta_t <- seq(0.001,0.5, length.out=n_out) #0.001 to 0.5 
 rho     <- seq(0,0.01, length.out=n_in)
 omega <- seq(0.001,1 , length.out=n_in) #note omega must be > rho, was 0.1
...
```{r contour_R0_2, message=FALSE,warning=FALSE }
 ...
 # eta_t <- seq(0.001,0.5, length.out=n_out) #0.001 
 # rho     <- seq(0,0.01, length.out=n_in)
 # omega <- seq(0.001,1 , length.out=n_in) #note omega must be > 0, was 0.1
...
4. Taylor expansion gives $R_0|_{(\rho=0,W_s=1)} = \beta/\gamma$.

5. Taylor expansion gives $R_0|_{(\rho=0,W_s=0)} = \beta/\gamma \frac{\gamma \tau(\omega+\gamma)W_s+\tau\rho(\gamma\eta_w+\omega\eta_tP_i)W_i+ \gamma \rho(\gamma+\omega)}{\gamma \tau(\omega+\gamma)W_s+\tau\rho(\gamma+\omega P_i)W_i+ \gamma \rho(\gamma+\omega)}$.

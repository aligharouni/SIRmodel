...
\end{equation}
across all susceptible pools with transmission rate $\beta$ and relative probability of transmission for ``waiting'' and \emph{confirmed positive} individuals, $\eta\_w$ and $\eta\_c$ respectively, (see Table~\ref{tab:params} for further details), (ii) $\eta\_c \leq \eta\_w$, i.e., the individuals awaiting test results have a higher transmission probability than the reported individuals. Note that for convential purposes in the interpretation of the results with respect to these isolation parameters, we define the transformed isolation parameters as $\theta\_w=1-\eta\_w$ and $\theta\_c=1-\eta\_c$. Thus, for instance when the awaiting people follow the isolation perfectly, $\theta\_w$ is closer to 1, while when they less follow the isolation, $\theta\_w$ is closer to 0. For this analysis, we also assume a perfectly specific test ($p_S=0$). This last assumption combined with the assumption that no individual is in waiting-for-\emph{positive} or \emph{confirmed positive} compartments, i.e., $S\_p(0)=S\_c(0)=0$, reduces the model to 10 equations (equations c and d  in \eqref{model} are eliminated).
...
\begin{tabular}{|c|L{2in}cc|} \hline
  ...
  $\eta\_w$ ($\theta\_w=1-\eta\_w$)  & Relative probability of transmission for ``waiting'' individuals & - & - \\ \hline
  $\eta\_c$ ($\theta\_c=1-\eta\_c$) & Relative probability of transmission for ``confirmed positive'' individuals & - & -  \\ \hline
  ...
  \hline
  \end{tabular}
\caption{\label{tab:params} Parameters of the model \eqref{model}.\todo{figure out the best way to put the new definition $\theta=1-\eta$} in the table.}
\end{table}
...
\end{equation}
For simplification of the analysis, we adopt the following change $x\_u^* = S\_u^*/N_0$ and $x\_n^* = S\_n^*/N_0$.
...
A=& \gamma \big((\omega+\gamma)+\omega p_I F_I \big) \big(x_n^*+\gamma(\omega+\gamma)F_I \big)\\
B=& \omega p_I F_I (\gamma x_u^*+\omega) \\ 
C=& \frac{1}{(\omega+\gamma) \big(\gamma(\omega+\gamma)+F_I(\gamma+\omega p_I) \big)}.
\end{aligned}
\end{equation}
% \begin{equation}
% \label{R0}

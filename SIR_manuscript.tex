...
Proposition \ref{prop1} part \ref{p1:omega} is the most interesting result. It states that returning test results more rapidly, i.e., increasing $\omega$, does not necessarily lower the reproduction number $\Rnum$; whether increasing $\omega$ lowers $\Rnum$ depends on the precise combination of model parameters (test reporting rate, testing strategies or test-specific testing weights, test sensitivity and specificity and isolation. 
Specifically, in the case of perfect isolation, i.e., when $\eta_w=0$ and $\eta_c=0$, $\Rnum$ may increase as the test reporting process become faster. This can be seen from expression \eqref{Rom}.
The interpretation for this phenomenon is that individuals are highly likely to fully self-isolate when they are awaiting test results and when they are reported positive with a negligable influence on the force of infection, $\lambda$. Thus the faster the test reporting rate, the shorter they stay in these ``\emph {safe}'' awaiting compartments, and the more they get involved in the infection process. 
Another example of this situation, when the slow test reporting rate would be favorable is in the case when the test being employed produces many false negatives. Because many infected individuals will believe they are uninfectious and thus may unknowingly spread the virus to many others. 

Our model enables us to quantify the amount of delay required in the test reporting process as a strategy to reduce $\Rnum$.

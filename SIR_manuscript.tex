...
\newcommand{\testtarget}{\ensuremath{\frac{w_I}{w_S}}\xspace}
...
\david{It would be better to say ``symptomatic'' rather than ``infected'' in the last sentence, but we can't claim the model really captures that.}

% %%%%%%%
\section{Introduction}

The observed dynamics of the \covid epidemic have been driven both by epidemiological processes (infection and recovery) and by testing processes (testing and test reporting). In addition to shaping epidemic observations (via case reports), testing processes also alter epidemiological dynamics \ali{\citep{peto2020covid,taipale2020population}}. Because individuals with confirmed infections (positive tests) are likely to self-isolate, and individuals who are awaiting the results of a test may also do so, testing will generally increase the number of people who are isolating and hence reduce epidemic growth rates. We developed a mechanistic model that incorporates epidemic processes and testing in order to explore the effects of testing and isolation on epidemic dynamics.
...
\david{The intro and discussion would benefit from (potentially many) additional references.}
Although testing and tracing is a key part of infection control strategies, mathematical epidemiologists have typically analyzed it with detailed models designed to inform particular public health efforts \ali{\citep{endo2020implication,hellewell2020feasibility,jenness2020modeling}}, rather than analyzing simple but general models of the feedback between testing and transmission dynamics.
\ali{the following sentences added to address Ben's comment:} There have been several modeling studies of testing and tracing dynamics and their interaction with epidemiological dynamics. In the context of repeated screening and random testing of isolated populations (such as a university), \cite{bergstrom2020frequency} provided some analytical results which suggests, as an outbreak control strategy, that proactively screening asymptomatic individuals and isolating confirmed-positive cases would reduce the spread of the disease. In particular, They explored the effects of testing frequency and Trade-offs between frequency, accuracy and the reporting delay on the epidemic dynamic (more specifically, on the effective reproductive number).
\cite{rogers2021high} simulated a SEIR model with testing and isolation which suggest a combination of rapid testing with antigen tests and the subsequent isolation of confirmed-positive individuals
a strategy for controlling the disease dynamics in the \covid pandemic. Further, \cite{friston2021testing} model the effects of self-isolation on testing and tracing with the focused on posterior predictions and projections under different testing and tracing scenarios. They conclude that the emergence of a second wave depends almost exclusively on the rate at which immunity is lost and that it is necessary to track the 20\% of asymptomatic individuals in order to control the outbreak. Our modeling approach is novel compared to these works as it mechanistically incorporates the epidemic and testing processes with different levels of testing focus from random to highly targeted. We hope that the present paper will inspire further explorations of the fundamental properties of epidemic models that incorporate explicit testing processes.
...
\bmb{eqs A2 and A3 can be in the same array environment, but sticking the ``and'' in there is ugly.
  ``and'' should be in text, with A4 set as a separate equation (it doesn't need to be aligned).}
\begin{align}
\label{FV}
...
Note that $\rho$ and $\omega$ have a similar mechanism in delaying people to get into $I\_c$, thus we would expect to see the non-trivial countervailing effect of these two parameters on $\Rnum$. \bmb{I don't understand the last sentence}
...
We define $w\_{IS}=\testtarget$. \bmb{shouldn't we use this simplified notation \textbf{throughout} the paper???}
\begin{align}
\label{eq:d_del_wis}

...
We derived general expressions for these derivatives. However, we analyzed the effect of $\omega$ on $\Rnum$ for the special case of low testing intensity. Specifically, by making the restriction $\rho \ll 1$, we are able to Taylor-expand $\Delta$ at $\rho=0$ and analyze the resulting simplified derivatives to illustrate a surprising non-monotonic relationship between $\Rnum$ and $\omega$. \bmb{The previous version of this implied that the restriction was $\rho \ll N_0$; that seems very odd since $\rho$ has units of time$^{-1}$. I think $\rho \ll 1$ is the correct statement (i.e. $\rho$ is already a \emph{per capita} measure), but please check. If I am wrong and $\rho$ is expressed in absolute units of tests/day, then the range quoted in the caption for \fref{pan} is weird \ldots}
...
\item \label{p1:w} Increasing testing focus, i.e. changing the testing weights from random ($w_S=w_I$) toward targeted  ($w_S<w_I$), always decreases \Rnum.
\end{enumerate}
...
\bmb{Perhaps we can also include baseline $\Rnum$ and $r$ (or doubling time), i.e. $\beta/\gamma$ and $\beta-\gamma$. (Why did we pick these particular beta/gamma values?)
If $\rho$ is per capita then does the figure depend on $N_0$ at all/can we leave it out of the list of specified parameters?}
}
\label{pan}
\end{figure}
...
\item We also see that greater test intensity $\rho$ increases the effectiveness of control $\Delta$ ($\Delta$ increases as we move up in each plot in this Figure). Mathematically speaking, we did find a countervailing effect, but this can only be seen when we allow $\rho$ to take unrealistically large values, and only for weighted/targeted testing (see \fref{pan2}). The explanation here is that more rapid testing leaves more susceptibles in the ``waiting-for-no'' category at the DFE; these people must then wait for their tests to be returned before they can be tested again, receive a positive test, and become cautious. This effect is usually weak compared to the helpful effects of testing, but if testing is weighted, $\theta_w$ is low, and test returns are slow, it is possible for increasing $\rho$ to reduce $\Delta$ (see upper-right part of upper-right plot of \fref{pan2}(b)).

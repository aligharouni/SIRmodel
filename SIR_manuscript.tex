...
\begin{equation}
\label{eq:del4}
\Delta= \frac{1}{N_0\,C}\big(C_1\,S\_u^*+((C_1+C_2)(1-\theta\_w)+C\,\theta\_w)\,S\_n^*\big).
\end{equation}
Note that for analysis convention, we let $N_0=1$, thus $S\_u^*$ and $S\_n^*$ are in the scale of 0 to 1. 
To derive the first result, i.e., how changes in $\theta\_w$ and $\theta\_c$ affects $\Rnum$, we factor these parameters in $\Delta$ \eqref{eq:del4}. Thus, $\Delta$ can be rewritten as
\begin{equation}
\label{eq:del4_theta}
\Delta= \frac{1}{C}\Big(
...
\pder[\Delta\,C]{\theta\_w}\bigg\rvert_{\theta\_w=1} =&

...
We used R \citep{r} for simulations and for plotting $\Rnum$ as a function of the underlying parameters. In particular, the contours of $\Delta$ \eqref{eq:Delta1} were plotted as in a range of a selected set of parameters of interest, $1/\omega$, $\rho$, $\theta\_w$ and $\theta\_c$, with the other parameters values kept fixed and presented in Table~\ref{tab:params}.

and isolation efficacy parameters, $\theta\_w$ and $\theta\_c$ vary from 0 to 1.

...
\subsection{More testing rate reduces $\Rnum$}
 ...
 The expression of the basic reproduction number $\Rnum$, given in Eqs. \eqref{R0}, makes it straight  forward to conclude $\frac{\partial \Delta}{\partial \rho} > 0$ and thus $\frac{\partial \Rnum}{\partial \rho} < 0$. 
Given the fractional form of $\Delta$, it is easy to show that $\partial A/\partial \rho >0$, $\partial B/\partial \rho >0$ and $\partial C/\partial \rho <0$. Note that we use the fact that
$\partial F_I/\partial \rho >0$, $\partial S\_n^*/\partial \rho >0$ and $\partial S\_u^*/\partial \rho <0$.

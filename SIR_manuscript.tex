...
We developed a deterministic model \eqref{model} which groups individuals based on disease status and testing status. The former includes Susceptible, Infectious and Recovered, thus SIR, and the latter categorizes people as \emph untested, waiting-for-\emph positive, waiting-for-\emph negative, or reported posi{}\emph tive [[JD: would it be better to use $c$ confirmed? Ali: yes]]. Also, two 'accumulator' compartments, $N$ and $P$, were incorporated in the model in order to collect cumulative reported negative or positive tests. The model \ref{model} and details of calculation of $\Rnum$ are presented in the Supplimentary Materials. The flowchart model is as follows 

\begin{figure}[!h] 
\begin{center} 
\includegraphics[scale=0.3]{./pix/sir_comp.png}
\caption{\small Flowchart of the SIR (Susceptible-Infectious-Recovered) model \eqref{model}. [Fady] Say more about the compartments and parameters here to make it stand on its own. The equations of the model are in Supplimentary Materials.
\label{fig:flowchart}}
\end{center} 
\end{figure}

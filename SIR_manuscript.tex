...
Much effort has been invested in developing widely accessible and fast tests in order to mitigate the spread of the pandemic (refs). In order to have a sense and as a comparison of slow versus fast test, we provide an example. The viral PCR test, which is the most commonly used test and takes about four days, is very slow relative to the potential tests such as breath-based test \citep{ruszkiewicz2020diagnosis}, which is still in trial stage and takes a few seconds. The point to be made is that there can be a potential advantage of slow test.

In this work we developed a model to incorporate epidemic processes and testing processes mechanistically to study the potential effect of testing strategies and slow-fast testing. The analysis of the deterministic model included deriving the basic reproduction number, $\Rnum$, as the index of threshold dynamics and the effect of underlying parameters on this key index. We found that the non-random testing strategy is more effective than the random testing. Specifically, the required critical testing intensity to keep $\Rnum$ lower than the threshold value of one is much lower in the non-random testing than the random one. Further, we show that for certain parameter combinations, there may be a potential advantage to slow reporting of test results on the spread of the infection.  


% %%%%%%%
\section{Method}

We developed a deterministic model as a platform of our study. A generic individual is identified in a model compartment based on the disease-specific and test-specific criteria. The former criteria include Susceptible, Infectious and Recovered, thus SIR, and the latter criteria group people in either untested, awaiting for the test result with expectation of positive or negative outcome, or reported positive, thus $u$, $p$, $n$ and $t$, respectively, reflected as subscript. Also, two 'integrator' or 'accumulator' compartments, $N$ and $P$, were incorporated in the model in order to collect cumulative reported negative or positive tests. The flowchart model is as follows 
...
where $\beta$, (in units of 1/day), is the disease transmission rate, $\eta_w$ and $\eta_t$ are the isolation parameters for awaiting and reported individuals, respectively. $\Lambda$ is the force of infection and is defined as 
...
Some of the underlying assumptions of the model \eqref{model} are as follows: (i) all groups are being exposed to all others, (ii) there is a single force of infection, $\Lambda$, that is, where a susceptible individual moves to does not depend on who infected it, (iii) a perfectly specific test, $p_S=0$, for simplicity in analysis, (iv)  $\eta_t \leq \eta_w$, i.e., the individuals awaiting test results have a higher transmission probability than the reported individuals. 
...
Further details are provided in the Appendix.
 ...
 [Fady? \fady{I didn't do any simulations though}] 
...
\end{proposition}
 ...
 Of these results, prop. \ref{p1:omega} is the most surprising. It states that returning test results more rapidly (i.e., increasing $\omega$) does not necessarily lower $\Rnum$, rather, whether increasing $\omega$ lowers $\Rnum$ is dependant on the precise combination of model parameters (test reporting rate itself, testing processes/strategies ($W$'s), testing efficacy ($P_I$) and isolation ($\eta$'s). Specifically, the $\partial{R_0}/\partial{\omega}>0$ suggests that reproduction number may increase as the test reporting proccess become faster. This can be seen from expression \eqref{Rom} in the case of ``perfect isolation``, when $\eta_w=0$ and $\eta_t=0$. The underlying mechanism for this result is that individuals may take more precautionary behaviours (e.g., physical distancing) when they are awaiting test results compared to when they are untested or their test has returned negative. An example of a situation that would favour slow return of tests is if the test being employed produces many false negatives, because many infected individuals will believe they are negative and thus may unknowingly spread the virus to many others.  For an example of the behaviour of $\Rnum$ see Figure \ref{pan} at varying values of $\omega$. This property also provides a tool to quantify the amount of delay required in the test reporting process as a strategy to reduce $\Rnum$ and consequently control the epidemic.   

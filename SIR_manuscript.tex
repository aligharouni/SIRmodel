...
\left[ \begin {array}{c}  x\_n^* \\ x\_u^* \\ 0 \\ 0  \end {array} \right]
...
The hard thing to prove is to show the reduction of $\Rnum$ with respect to testing intensity, $\rho$, and the speed of the test return, $\omega$, for the feasible ranges of the parameters, i.e., $0<\omega$, $0 \leq \rho<\omega$, and $0 \leq \theta\_w \leq \theta\_c \leq 1$.
...
% %%%%%%%
\subsection{More testing rate reduces $\Rnum$}
...
We will show that (a) $\pro (A/C)>0$ and (b) $\pro (B/C)>0$.
Note that we will use the following information: 
\begin{enumerate}
\item
$\pro \hat F_I >0$ for  all $\rho$ and specifically the feasible range, $\rho \in [0,\omega]$. Here $\hat F_I = (\omega\rho/(\omega-\rho))w_I/w_S$ for the \percap testing rate of infectious individuals near the disease-free equilibrium,
\item $\pro \hat F_I x\_u^* >0$ since $\hat F_I x\_u^* = \rho w_I/w_S$,
\item $\pro \hat F_I x\_n^* >0$ since $\hat F_I x\_n^* = \frac{\rho^2}{\omega-\rho} w_I/w_S$, 
\item $\pro x\_n^* >0$ and $\pro x\_u^* <0$ given the DFE expression in \ref{dfe},
\item
\begin{align}
\pro A &= 
\gamma \Big(\gamma(\omega+\gamma)\pro x\_n^* + \omega p_I \pro(\hat F_I x\_n^*)+(\omega+\gamma) \pro(\hat F_I)
\Big),\\
\pro B &= \gamma \omega p_I \pro(\hat F_I x\_u^*)+\omega^2 p_I \pro(\hat F_I),\\
\pro C &=(\omega+\gamma)(\omega p_I+\gamma) \pro(\hat F_I).
\end{align}
\end{enumerate}
...
\begin{align}
& \pro (A/C) = \pro{A} \dot C - \pro{C} \dot A \\
&= \gamma (\omega+\gamma)^2(\omega p_I+\gamma) 
\Big( 
[\gamma \pro x\_n^* + \frac{\omega p_I}{(\omega+\gamma)} \pro(\hat F_I x\_n^*) + \pro\hat F_I]
[\gamma\frac{(\omega+\gamma)}{(\omega p_I+\gamma)}+\hat F_I] - \\
& \pro(\hat F_I)
[\gamma x\_n^* + \frac{\omega p_I}{(\omega+\gamma)}(\hat F_I x\_n^*) + \hat F_I] 
\Big) 
\end{align}

Since $\pro(x\_n^*) >0$ and $\pro (\hat F_I x\_n^*)>0$,  it is enough to show that the coefficient of 
$\pro (\hat F_I)$ is summed up to a positive value.
$\pro (\hat F_I)$ coefficient is as follows
$$\big( \frac{\omega p_I}{(\omega+\gamma)} x\_n^* +1 \big) 
\big( \gamma\frac{(\omega+\gamma)}{(\omega p_I+\gamma)} + \hat F_I \big) 
-
\big( \gamma x\_n^* + \frac{\omega p_I}{(\omega+\gamma)}(\hat F_I x\_n^*) + \hat F_I 
\big).
$$
After simplification, we use the fact that the following expression is positive
$$(\frac{(\omega+\gamma)}{(\omega p_I+\gamma)}) (1+ \frac{\omega p_I x\_n^*}{(\omega+\gamma)}) -1. $$
This is because $(\frac{(\omega+\gamma)}{(\omega p_I+\gamma)})>1$ and  $(1+ \frac{\omega p_I x\_n^*}{(\omega+\gamma)}) >1$.

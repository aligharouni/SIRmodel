...
\ali{the following sentences added to address Ben's comment:} There have been several modeling studies of testing and tracing dynamics and their interaction with epidemiological dynamics. In the context of repeated screening and random testing of isolated populations (such as a university), \cite{bergstrom2020frequency} provided some analytical results\david{what are the results?  that's what is most important, not whether the results were analytical}, \cite{rogers2021high} simulated a SEIR model with high-low \david{meaning?} testing intensity and sensitivity\david{what did they find?}. Further, \cite{friston2021testing} model the effects of self-isolation on testing and tracing\david{what did they find?}. Our modeling approach is novel compared to these works\david{how so?  You can't just say our work is novel.  What makes it novel relative to these previous papers?}. We hope that the present paper will inspire further explorations of the fundamental properties of epidemic models that incorporate explicit testing processes.

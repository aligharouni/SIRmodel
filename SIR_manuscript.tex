...
\begin{align}
\label{eq:dd2dr}
\pro \Delta=& \frac{1}{d} (a\,\rho^2+b\,\rho+c), \text{where}\\
...
\end{align}
with scaler $d>0$ (not effecting the sign of $\pro \Delta$) and $D_1$ and $D_2$ as 
\begin{align}
\label{eq:D1D2}
...
& \omega>0, \\
& 0 \leq \rho<\omega,\\ 
& 0 \leq \theta\_w \leq \theta\_c \leq 1, and 
& \frac{w_I}{w_S}\geq 1.
\end{align}
Given the parameter restrictions, we would like to determine the sign pattern of $\pro \Delta$ given the quadratic form of it in Eq. \eqref{eq:dd2dr}. Thus, it is important to know the sign of the coefficients $a$, $b$ and $c$ of the polynomial in Eq. \eqref{eq:dd2dr}. Since $D_1\geq 0$ and $D_2\geq 0$, $b\leq 0$ and $c\geq 0$. Sign of $a$ depends on the interplay between the involved parameters. Note that $D_1 \leq D_2$. (\todo{still working on}).

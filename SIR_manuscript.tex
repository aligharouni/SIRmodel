...
\begin{equation}
\label{eq:R0appr}
\Rnum \approx \beta/\gamma + \frac{\beta \rho}{\omega (\omega+\gamma) \gamma^2 w_S} \Big(\gamma(-\theta\_w)(\gamma w_S+\omega w_I) + (-\theta\_c)p_I w_I \omega^2 \Big) + \mathcal{O}(\rho^2).
\end{equation}
...
\end{equation}
where $a=(-\theta\_w)w_I-(-\theta\_c)p_I w_I = ((s-p_I)(1-\theta\_c) + (p_I-1)) w_I$, $b=2(-\theta\_w)\gamma w_S$ and $c=(-\theta\_w)\gamma^2 w_S$.
Given that $0 \leq \theta\_w \leq \theta\_c \leq 1 $, one can easily derive $b\leq 0$ and $c \leq 0$. 

Note that in general, the necessary and sufficient condition for $a \geq 0$ is $(s-p_I) (1-\theta\_c) \geq (1-p_I)$, where $s=\frac{(1-\theta\_w)}{(1-\theta\_c)} \geq 1$. 

In the case of ``perfect isolation'', i.e., when $\theta\_w=1$ and consequently $\theta\_c=1$, it is straightforward to see that $a \leq 0$, $b<0$ and $c<0$. Thus, $\partial{\Rnum}/\partial{\omega} \geq 0$. 

...
These parameters include the isolation efficacy parameters, $\theta\_c$ and $\theta\_w$, \percap testing intensity, $\rho$, and the rate of test return, $\omega$. In partticular, we look at the partial derivatives of $\Delta$ with respect to these parameters (see details in appendix, Sec.\,\ref{app:rho}). 
It is notable that we study the effect of $\omega$ on $\Rnum$ for the case when testing intesity is very small by using the Taylor approximation of $\Rnum$ at $\rho=0$.  
...
We used R \citep{r} for simulations, in particular,  for plotting the contours of $\Delta$ \eqref{eq:del4} over a range of selected set of parameters of interest, i.e., parameters that could be manipulated by public-health policy. These parameters include the isolation efficacy parameters, $\theta\_c$ and $\theta\_w$, \percap testing intensity, $\rho$, and the rate of test return, $\omega$.

...
The Disease-Free Equilibrium (DFE) for the expanded SIR model (Eqs.~\ref{model}) is found by setting the infected compartments to 0 and solving for the unknowns. \rev {The DFE depends on \percap testing intensity $\rho$, the rate of test return $\omega$, and the population size $N$}. The DFE is 
\begin{equation}
\label{dfe}
